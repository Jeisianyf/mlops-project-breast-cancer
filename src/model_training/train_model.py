...
def train_model(train_data: pd.DataFrame, params: dict[str, int | float]) -> None:
    ...
    Args:
        ...
        params (dict[str, int | float]): Model hyperparameters.
    """
    mlflow.set_experiment("ml_breast_cancer_classification")

    mlflow.keras.autolog()

    is_experiment_dvc = os.getenv("DVC_EXP_NAME") is not None
    extra_args = {}
    if is_experiment_dvc:
    ...
    with mlflow.start_run(**extra_args):
        ...
        # Create the model
        model = create_model(
            input_shape=X_train.shape[1], num_classes=1, params=params
        )
